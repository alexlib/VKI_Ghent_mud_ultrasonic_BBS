...
def multipass_img_deform(
    ...
    ip = RectBivariateSpline(y_old, x_old, u_old.filled(0.),kx=interpolation_order,ky=interpolation_order)
    u_pre = ip(y_int, x_int)

    ip2 = RectBivariateSpline(y_old, x_old, v_old.filled(0.),kx=interpolation_order,ky=interpolation_order)
    ...
    if settings.deformation_method == "symmetric":
        ...
        x_new, y_new, ut, vt = create_deformation_field(
            frame_a, x, y, u_pre, v_pre, interpolation_order=settings.interpolation_order)
